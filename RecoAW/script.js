var postsLoader = {
	...
	postJSON: [
		    {
		        "type": "answer",
		        "tnail": "Deep Copy | Copy constructors | Reflection",
		    ...
		    },
		    {
		        ...
		        "tnail": "Inheritance | Class | Polymorphism | Private",
		    ...
		    {
		        "type": "answer",
		        "tnail": "Polymorphism | Inheritance | Instance | Interface",
		    ...
		    {
		        "type": "question",
		        "tnail": "Java | Abstract | Class | Code",
		    ...
		    {
		        "type": "answer",
		        "tnail": "Deep Copy | Serialize | Stream",
		    ...
		    {
		        "type": "question",
		        "tnail": "Deep Object Copy | Clone | Reference",
		    ...
		    {
		        "type": "answer",
		        "tnail": "Synthetic Classes | Dynamic Proxies | Spring | Libraries | Runtime",
		    ...
		    {
		        "type": "answer",
		        "tnail": "Identifier | Session | Client",
		    ...
		    {
		        "type": "accepted-answer",
		        "tnail": "Serialization | Singleton | Class",
	...
	load: function() {
		var json = this.postJSON;
		for (var i=0;i<json.length;i++) {
			this.addPostToPage(json[i], i);
		}

	},
	processRecommendation: function(key) {
		var reco = postsLoader.recommendations[key];
		var recoMarkup = "<ol class='recoList''>";
		if (reco) {
			for (var i=0; i<reco.length; i++) {
				recoMarkup += "<li>" + reco[i] + "</li>";
			}
		} else {
			recoMarkup += "No recommendations";
		}
		recoMarkup += "</ol>";
		return recoMarkup;
	},
	addPostToPage: function(curPost, i) {
		var quarterOrHalf = (curPost.type == 'question') ? 'half' : 'quarter';
		var $post = $("<div>", {id: "post_"+i, class: "post " + curPost.type + " " +quarterOrHalf});
		//replace recommendation with recommendation for post. Can be made into a markup too.
		curPost.recommendation = this.processRecommendation("Post "+ (i+1));

		var html = "<div class='closer' onclick='postsLoader.seeLess("+i+")'>X</div>"
					+ "<div class='front' onclick='postsLoader.seeMore("+i+")'>"
						+ "<div class='"+ curPost.type +"-inner post-inner'>"
						+    "<div class='post-title'>"+ curPost.type+"</div>"
						+    "<div class='content'>" + curPost.tnail +"</div>"
						//+    "<div class='toggler' id='postToggler_"+i+"'>See More</div>"
					    + "</div>"
				   + "</div>"
				   + "<div class='back " + curPost.type + "-back'>"
						+ 	"<div class='"+ curPost.type +"-inner post-inner'>"
							+		"<div class='post-content'>"
							+		"<div class='post-title2'>Post -" + (i+1) + "</div>"
							+ 		"<div>" + curPost.text  +"</div>"
							+		"</div>"
							+       "<div class='content'>"
							+			"<h4 style='text-decoration:underline;'> Recommendations </h4>"
							+           curPost.recommendation
							+        "</div>"
						+   "</div>"
						//+ "<div onclick='postsLoader.seeLess("+i+")' class='toggler'>Back</div>"
				   +"</div>"

		$post.html(html);
		$("#wrapper").append($post);
		//$post.click(function(){postsLoader.postClickedNew(i)});
	},
	...
	closeSidebar: function() {
		...
		$('#wrapper').removeClass('wrapper-sidebar-open');	
	}

}

$(document).ready(function() {
	$.getJSON( "recoOutput.json", function( data ) {
		postsLoader.recommendations = data;
	});
	var poll = setInterval(function(){
		if (postsLoader.recommendations !== null) {
			postsLoader.load();
			clearInterval(poll);
		}
	},1000);
